...
# Parsing Errors mean that R has trouble determining what a column of data
# should be stored as
```
...
# clean_names() changes the column/variable names from capitalized to all lowercase, removes and changes punctuation and spaces, etc to make it more code friendly.
...
Important: 
The %>% tidyverse pipe operator is used to apply functions to an object, 
whereas + signs are exclusively used in ggplot to add on to a ggplot visual.

Command + shift + m = add in a tidyverse pipe operator %>%
...
# Assumes the last created graph as the one being exported
# can add "plot = name" for the specific plot/object ex: "plot = toolik_temp_plot"
```
